...
This directory contains Dockerfiles and Compose assets for running project tests with Temporal and Prometheus.

## Files

1. `docker-compose.projecttests.yml`: Temporal dev server, UI, Prometheus, and Go runner/worker services.
2. `projecttest-go-base.Dockerfile`: reusable base image (omes CLI + Go toolchain + harness).
3. `projecttest-go.Dockerfile`: thin Go project overlay image.
...
Build the project test base image:

```bash
docker build -f projecttests/dockerfiles/projecttest-go-base.Dockerfile \
  -t omes-projecttest-go-base:local .
```

Optional shell setup:

```bash
export COMPOSE_FILE=projecttests/dockerfiles/docker-compose.projecttests.yml
```

If you do not set `COMPOSE_FILE`, add `-f projecttests/dockerfiles/docker-compose.projecttests.yml` to each compose command.
...
docker compose run -d --name omes-worker --service-ports worker-go \
  exec \
  --language go \
  --project-dir /app/projecttests/go/tests/helloworld \
  -- \
  worker \
...
1. `omes project ... --spawn-worker`
2. a separate local worker process + `omes project ...` without `--spawn-worker`
