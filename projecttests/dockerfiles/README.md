...
2. `workflowtest-go.Dockerfile`: reusable Go base image for workflow tests.
...
Build the Go workflowtest base image:

```bash
docker build -f projecttests/dockerfiles/workflowtest-go.Dockerfile \
