...
void test('runWorker passes shared client and context to each worker factory', async () => {
  ...
  const clientFactory = async (_config: ClientConfig): Promise<Client> => {
    return client;
  };
  ...
  assert.deepEqual(
    workerFactoryCalls.map(({ context }) => ({
...
void test('runWorkers shuts down all workers when one fails', async () => {
  ...
  await assert.rejects(
    () =>
      runWorkers(
        [failingWorker as unknown as Worker, successfulWorker as unknown as Worker],
        neverInterrupt(),
      ),
    /boom/,
  );

  assert.equal(failingWorker.runCalls, 1);
