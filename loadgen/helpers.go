...
func InitSearchAttribute(
	...
	info.Logger.Infof("Initialising Search Attribute %s", attributeName)
	...
	if errors.As(err, &alreadyErr) {
		info.Logger.Infof("Search Attribute %s already exists", attributeName)
	} else if err != nil {
		info.Logger.Warnf("Failed to add Search Attribute %s: %v", attributeName, err)
		if !errors.As(err, &deniedErr) {
			return err
		}
	} else {
		info.Logger.Infof("Search Attribute %s added", attributeName)
	}
