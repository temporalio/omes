...
func (e *ebbAndFlowProjectExecutor) Run(ctx context.Context, info loadgen.ScenarioInfo) error {
	...
	// Context setup (matches saturation_executor.go pattern).
	runCtx, cancelRun := context.WithCancel(ctx)
	defer cancelRun()
	if info.Configuration.Timeout > 0 {
		var cancelTimeout context.CancelFunc
		runCtx, cancelTimeout = context.WithTimeout(runCtx, info.Configuration.Timeout)
		defer cancelTimeout()
	}
	durationCtx, cancelDuration := context.WithTimeout(runCtx, info.Configuration.Duration)
	...
	var nextIteration atomic.Int64
	nextIteration.Store(1)

	// Error channels.
	resultCh := make(chan error, 10000)
	...
	go func() {
		...
		for {
			...
			case <-runCtx.Done():
				return
			case <-ticker.C:
				// Drain result channel to track consecutive errors.
				for {
					select {
					case err := <-resultCh:
						if err != nil {
							...
							if consecutiveErrors >= config.MaxConsecutiveErrors {
								sendFatalErr(fmt.Errorf("got %d consecutive errors, most recent: %w",
			...
			drained:
				...
				if activities > 0 {
					payload, err := json.Marshal(ebbAndFlowPayload{ActivityCount: activities})
					if err != nil {
						sendFatalErr(fmt.Errorf("failed to marshal payload: %w", err))
						return
					}

					iter := nextIteration.Add(1) - 1
					...
					go func(iter, activities int64, payload []byte) {
						...
						_, execErr := e.client.Execute(runCtx, &api.ExecuteRequest{
							Iteration: iter,
							Payload:   payload,
						})
	...
	// Backlog logging goroutine.
	go func() {
		logTicker := time.NewTicker(config.BacklogLogInterval)
	...
	// Wait for duration or fatal error.
	select {
	case err := <-errCh:
		cancelRun()
		workersWG.Wait()
	...
	for {
		...
		case <-done:
			...
			e.completedWorkflows = int(nextIteration.Load() - 1)
