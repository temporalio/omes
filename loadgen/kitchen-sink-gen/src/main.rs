...
impl<'a> Arbitrary<'a> for ClientActivity {
    fn arbitrary(_u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        // Create a simple client sequence with just a query action to avoid recursion
        let client_sequence = ClientSequence {
            action_sets: vec![ClientActionSet {
                actions: vec![ClientAction {
                    variant: Some(client_action::Variant::DoQuery(DoQuery {
                        variant: Some(do_query::Variant::ReportState(Payloads { payloads: vec![] })),
