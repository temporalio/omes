...
mod tests {
    ...
    #[test]
    fn nexus_operation_generator_covers_sync_and_async_operations() {
        let mut saw_sync = false;
        let mut saw_async = false;

        for byte in 0..=u8::MAX {
            let data = vec![byte; 1024];
            let mut unstructured = Unstructured::new(&data);
            let Ok(operation) = ExecuteNexusOperation::arbitrary(&mut unstructured) else {
                continue;
            };

            match operation.input.and_then(|input| input.action) {
                Some(nexus_operation_request::Action::Echo(_)) => saw_sync = true,
                Some(nexus_operation_request::Action::WorkflowAction(workflow)) => {
                    ...
                    assert!(matches!(
                        actions[0].variant,
                        Some(action::Variant::ReturnResult(_))
                    ));
                    saw_async = true;
                }
                _ => {}
        ...
        assert!(saw_sync, "expected a generated synchronous Nexus operation");
        assert!(
            saw_async,
            "expected a generated asynchronous Nexus operation"
        );
    }
}
