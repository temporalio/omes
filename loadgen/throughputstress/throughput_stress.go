...
type (
	...
	WorkflowParams struct {
		...
		// If set, the workflow will run the SleepActivity with the given priority sleep patterns.
		// This is mutually exclusive with SleepActivity.
		SleepActivityWithPriorities SleepActivity[int] `json:"sleepActivityPriorities"`
	}
	...
	SleepActivity[T loadgen.Discrete] struct {
		// The distribution of sleep patterns.
		PatternsDist loadgen.DiscreteDistributionDist[T] `json:"patterns_dist"`
		// The distribution of sleep durations for each sleep pattern.
		PatternDurationsDist map[T]loadgen.ContiniousDistribution[time.Duration] `json:"pattern_durations_dist"`
	}
)
