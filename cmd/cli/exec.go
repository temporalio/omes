...
Examples:
  omes exec --language go --project-dir ./projecttests/go/tests/helloworld -- worker --task-queue q
  omes exec --language go --project-dir ./projecttests/go/tests/helloworld -- project-server --port 8080
  omes exec --language go --project-dir ./projecttests/go/tests/helloworld --process-monitor-addr :9091 -- worker --task-queue q`,
	...
	if languageFlag := cmd.Flags().Lookup("language"); languageFlag != nil {
		languageFlag.Usage = "Language to use for workflow tests (go only)"
	}
