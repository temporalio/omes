...
type projectRunner struct {
	...
	projectRunOpts clioptions.ProjectRunOptions
	metricsOpts  projectMetricsFlags
	runID        string
	runFamily    string
	taskQueue    string
	clientPort   int
...
type projectMetricsFlags struct {
...
func (r *projectRunner) addCLIFlags(cmd *cobra.Command) {
	...
	r.projectRunOpts.AddCLIFlags(fs)
	...
	fs.BoolVar(&r.doNotRegisterSearchAttributes, "do-not-register-search-attributes", false, "Do not register search attributes on init")
...
func (r *projectRunner) validate(cmd *cobra.Command) error {
	...
	if !r.projectRunOpts.SpawnWorker && !cmd.Flags().Changed("task-queue") {
	...
	if r.metricsOpts.WorkerProcessMetricsAddress != "" && !r.projectRunOpts.SpawnWorker {
...
func (r *projectRunner) run(ctx context.Context) error {
	...
	if r.projectRunOpts.SpawnWorker {
	...
	projectHandle, err := projectexecutor.NewProjectHandle(ctx, r.clientPort, &api.InitRequest{
		...
		Config:                   configBytes,
		RegisterSearchAttributes: !r.doNotRegisterSearchAttributes,
	})
	if err != nil {
