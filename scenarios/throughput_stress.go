...
const (
	...
	SleepActivityWithPrioritiesJsonFlag = "sleep-activity-with-priorities-json"
)
...
func (t *tpsExecutor) Run(ctx context.Context, info loadgen.ScenarioInfo) error {
	...
	var sleepActivityWithPriority throughputstress.SleepActivity[int]
	sleepActivitiesWithPrioStr := info.ScenarioOptions[SleepActivityWithPrioritiesJsonFlag]
	if sleepActivitiesWithPrioStr != "" {
		err := json.Unmarshal([]byte(sleepActivitiesWithPrioStr), &sleepActivityWithPriority)
		if err != nil {
			return fmt.Errorf("failed to parse %s: %w", SleepActivityWithPrioritiesJsonFlag, err)
		}
	}
	spew.Dump(sleepActivityWithPriority)
		...
		Execute: func(ctx context.Context, run *loadgen.Run) error {
			...
			err = run.ExecuteAnyWorkflow(ctx,
				...
				throughputstress.WorkflowParams{
					...
					SleepActivityWithPriorities:  sleepActivityWithPriority,
				})
